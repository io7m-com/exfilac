...
public final class EFQUploadConfigurationPut
  ...
  private static final String QUERY = """
    INSERT INTO upload_configurations (
      ...
      ?,
      ?,
      SELECT bucket_id FROM buckets WHERE bucket_name = ?,
      ?
    ) ON CONFLICT DO UPDATE SET
      upload_device_path = ?,
      upload_bucket_id   = SELECT bucket_id FROM buckets WHERE bucket_name = ?,
      upload_policy      = ?
    """;
  ...
  protected DDatabaseUnit onExecute(
    ...
    try (var st = connection.prepareStatement(QUERY)) {
      ...
      st.setString(2, upload.getDevicePath().toString());
      st.setString(3, upload.getBucket().getValue());
      st.setBytes(4, policyText);

      st.setString(5, upload.getDevicePath().toString());
